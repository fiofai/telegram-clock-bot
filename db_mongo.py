...
import ssl  # 导入SSL模块用于证书处理

# 设置日志
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# 禁用SSL证书验证，解决Render环境中的SSL握手问题
...
try:
    ...
    client = MongoClient(
        MONGO_URI,
        ssl=True,
        ssl_cert_reqs=ssl.CERT_NONE,  # 禁用证书验证
    ...
    logger.info("MongoDB连接成功，集合和索引已设置")
...
def get_driver_logs():
    ...
    try:
        ...
        logger.info(f"成功获取 {len(logs)} 个用户的打卡记录")
    except Exception as e:
        logger.error(f"获取打卡记录失败: {str(e)}")
        # 返回空字典而不是抛出异常，确保程序可以继续运行
    return logs
...
def save_driver_logs(logs):
    ...
    try:
        ...
        logger.info(f"成功保存 {len(logs)} 个用户的打卡记录")
    except Exception as e:
...
def get_driver_salaries():
    ...
    try:
        ...
        logger.info(f"成功获取 {len(salaries)} 个用户的薪资信息")
    except Exception as e:
...
def save_driver_salaries(salaries):
    ...
    try:
        ...
        logger.info(f"成功保存 {len(salaries)} 个用户的薪资信息")
    except Exception as e:
...
def get_driver_accounts():
    ...
    try:
        ...
        logger.info(f"成功获取 {len(accounts)} 个用户的账户信息")
    except Exception as e:
...
def save_driver_accounts(accounts):
    ...
    try:
        ...
        logger.info(f"成功保存 {len(accounts)} 个用户的账户信息")
    except Exception as e:
...
def save_single_driver_account(user_id, account_data):
    ...
    try:
        ...
        logger.info(f"成功保存用户 {user_id} 的账户信息")
    except Exception as e:
        logger.error(f"保存用户 {user_id} 的账户信息失败: {str(e)}")
        raise
...
def add_claim(user_id, claim_data):
    ...
    try:
        ...
        result = db.driver_accounts.update_one(
            {"user_id": user_id},  # 查询条件
            {
        ...
        logger.info(f"成功为用户 {user_id} 添加报销记录，金额: {claim_data['amount']}")
    except Exception as e:
        logger.error(f"为用户 {user_id} 添加报销记录失败: {str(e)}")
        raise
...
def add_topup(user_id, topup_data):
    ...
    try:
        ...
        logger.info(f"成功为用户 {user_id} 添加充值记录，金额: {topup_data['amount']}")
    except Exception as e:
        logger.error(f"为用户 {user_id} 添加充值记录失败: {str(e)}")
        raise
